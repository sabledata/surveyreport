...
```{r CatchRateText, echo=FALSE}
    ...
    strscurrent       <- strs01[strs01$SET_YEAR==yr+1,]
    strscurrentv      <- mean(sum(strscurrent$CPUE_SABLE_WEIGHT)/sum(strscurrent$CPUE_TRAPS))   # wt/trap, changed from TOTAL_TRAPS to CPUE_TRAPS
    
    strsyrb4          <- strs01[strs01$SET_YEAR==yr,]
    strsyrb4v         <- mean(sum(strsyrb4$CPUE_SABLE_WEIGHT)/sum(strsyrb4$CPUE_TRAPS))
    
    strs2yrb4          <- strs01[strs01$SET_YEAR==yr-1,]
    strs2yrb4v         <- mean(sum(strs2yrb4$CPUE_SABLE_WEIGHT)/sum(strs2yrb4$CPUE_TRAPS))
    
    percentchng1      <- round(((strscurrentv-strsyrb4v)/strscurrentv)*100,0) 
    percentchng2      <- round((strscurrentv-(strsyrb4v+strs2yrb4v)/2)/strscurrentv*100,0)
