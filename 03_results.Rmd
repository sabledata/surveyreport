...
```{r CatchRateText, echo=FALSE}
    cpueVar <- read.csv("c:/github/surveyreport/standaloneData/LandmarkCPUEVarianceCVs_StRS.csv",header=T)
    
    strscurrent       <- cpueVar[cpueVar$SET_YEAR==yr+1,]
    strscurrentv      <- round(strscurrent$yStr,0)
    
    strsyrb4          <- cpueVar[cpueVar$SET_YEAR==yr,]
    strsyrb4v         <- round(strsyrb4$yStr,0)
    
    strs2yrb4          <- cpueVar[cpueVar$SET_YEAR==yr-1,]
    strs2yrb4v         <- round(strs2yrb4$yStr,0)
    
    percentchng1      <- round(((strscurrentv-strsyrb4v)/strscurrentv)*100,0) 
    percentchng2      <- round(((strscurrentv-strs2yrb4v)/strscurrentv)*100,0)
