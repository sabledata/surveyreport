...
```{r appendixB}
   ...
   latcnt  <-  length(ssdat[, 9])
               for (i in 1:latcnt)
                      { ssdat[i,9]= paste("$",ssdat[i,9], "^o", ssdat[i,10],"$",sep="") }
   latcnt  <-  length(ssdat[,11])
               for (i in 1:latcnt)
                      { ssdat[i,11]=paste("$",ssdat[i,11],"^o", ssdat[i,12],"$",sep="") }
   latcnt  <-  length(ssdat[,13])
               for (i in 1:latcnt)
                      { ssdat[i,13]=paste("$",ssdat[i,13],"^o", ssdat[i,14],"$",sep="") }
   latcnt<-length(ssdat[,15])
               for (i in 1:latcnt)
                      { ssdat[i,15]=paste("$",ssdat[i,15],"^o", ssdat[i,16],"$",sep="") }
   ...
   column_spec(9,width    = "1.3cm") %>%  # lat
   column_spec(10,width   = "1.3cm") %>%
   column_spec(11,width   = "1.3cm") %>%
   column_spec(12,width   = "1.3cm") %>% 
...
# {#app:fourth-appendix} 
Summary of Sablefish biological data collected in the `r yr` survey by set.
...
```{r appendixD, echo=FALSE}
   ...
   surveyspec           <-  read.csv("c:/github/surveyreport/standaloneData/appendixD.csv",header=T)
...
Summary of Sablefish biological data collected in the `r yr+1` survey by set.

```{r appendixE, echo=FALSE}
   #  sql query for inline text
   #  Sablefish.dbo.procReport_Survey_SampleDetails pulls data from table GFBIO_RESEARCH_SAMPLE_DETAILS built by     
   #  procedure Sablefish.dbo.Build_BIO_Sablefish_Tables 
   samples       <-  paste("dbo.procReport_Survey_SampleDetails ",yr+1,",1,",setcnt, sep="")
   #surveyspec   <-  GetSQLData(samples,"Sablefish")
   surveyspec    <- read.csv("c:/github/surveyreport/standaloneData/appendixE.csv",header=T)
   
   colnames(surveyspec) <-  c("", "kg", "Count", "Recover-Rerelease", 
