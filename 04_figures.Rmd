...
#   query----------raw survey datafile for Landmark fisheries----------------
    ...
    raw_survey_data <- read.csv("c:/github/surveyreport/standaloneData/RawLandmarkSurveyData.csv",header=T)
    ...
    ggplot(strs.02,aes(x=FE_MODAL_BOTTOM_DEPTH,y=CPUE_SABLE_WEIGHT/TOTAL_TRAPS)) +
...
```{r figure7, fig.cap='(ref:figure7)', results='asis',echo=FALSE,fig.height = 6, fig.width = 7, fig.align = "center"}
    ...
    strs$sable_cpue        <- strs$CPUE_SABLE_WEIGHT/strs$TOTAL_TRAPS 
    strs$sable_cnt         <- (strs$CPUE_SABLE_COUNT/strs$TOTAL_TRAPS)
