...
```{r SensorText, echo=FALSE}
   ...
   srvyset  <-  paste("select Year, SUM(SBE39) AS SBE39, SUM(HOBO) AS HOBO, SUM(CTD) AS CTD, SUM(CAM) AS CAM ",
                      "from dbo.Report_SBE_HOBO_IND group by Year having (Year =", yr,")", sep="")
   #counter  <-  GetSQLData(srvyset,"Sablefish")
   counter  <-  read.csv("c:/github/surveyreport/standaloneData/hobo.csv",header=T)
   
   srvyset2 <-  paste("select Year, SUM(SBE39) AS SBE39, SUM(HOBO) AS HOBO, SUM(CTD) AS CTD, SUM(CAM) AS CAM ",
                      "from dbo.Report_SBE_HOBO_IND group by Year having (Year =", yr+1,")", sep="")
   #counter2 <-  GetSQLData(srvyset2,"Sablefish")
   counter2  <-  read.csv("c:/github/surveyreport/standaloneData/hobo.csv",header=T)
